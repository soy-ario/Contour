...
const KPI_CONFIG: { key: ChartMetric; label: string; icon: React.ReactNode }[] = [
  ...
  { key: "impressions", label: "Impressions", icon: <Monitor className="w-[18px] h-[18px] text-[#5B7A1A]" /> },
  { key: "engagement", label: "Engagement", icon: <MousePointer2 className="w-[18px] h-[18px] text-[#5B7A1A]" /> },
];
...
function getMetricTotalTop(items: TopContentItem[], key: "views" | "reach" | "impressions" | "engagement"): number {
  if (key === "engagement") {
    return items.reduce((sum, s) => sum + s.likes + s.comments + s.shares + s.saves, 0);
  }
  return items.reduce((sum, s) => sum + s[key], 0);
}
...
export default function AnalyticsPageContent({
  ...
  const filteredSnapshots = React.useMemo(() => {
    ...
    const dates = data.map((s) => ({ start: new Date(s.periodStart).getTime(), end: new Date(s.periodEnd).getTime() }));
  ...
  }, [allSnapshots, filterClient, dateRange]);

  const filteredTopContent = React.useMemo(() => {
    if (filterClient === "all") return allTopContent;
    return allTopContent.filter((c) => c.clientId === filterClient);
  }, [allTopContent, filterClient]);

  const previousPeriodSnapshots = React.useMemo(() => {
    if (filteredSnapshots.length === 0) return [];
    const dates = filteredSnapshots.map((s) => ({ start: new Date(s.periodStart), end: new Date(s.periodEnd) }));
  ...
  }, [allSnapshots, filteredSnapshots, filterClient]);
  ...
  const platformBreakdown = React.useMemo(() => {
    const map = new Map<string, { views: number; reach: number }>();
    filteredSnapshots.forEach((s) => {
      const existing = map.get(s.platform) || { views: 0, reach: 0 };
  ...
  const chartData = React.useMemo(() => {
    const map = new Map<string, { date: string; views: number; reach: number; impressions: number; engagement: number }>();
    filteredSnapshots.forEach((s) => {
      const key = formatDate(s.periodStart, "MMM dd");
      const existing = map.get(key) || { date: key, views: 0, reach: 0, impressions: 0, engagement: 0 };
      existing.views += s.totalViews;
      existing.reach += s.totalReach;
  ...
  const handleExport = () => {
    const rows = [["Client", "Platform", "Period Start", "Period End", "Views", "Reach", "Impressions", "Engagement"]];
    filteredSnapshots.forEach((s) => {
      rows.push([
        s.clientBrandName,
  ...
  const availableDates = React.useMemo(() => {
    if (allSnapshots.length === 0) return { min: new Date().toISOString(), max: new Date().toISOString() };
    const dates = allSnapshots.map((s) => new Date(s.periodStart).getTime());
    return {
      ...
      <div className="max-w-[1440px] mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 h-10 px-3.5 rounded-xl border border-[#ECECF4] bg-white text-sm text-[#6B7280] w-[240px]">
            ...
            <span className="truncate">
              {dateRange
                ? `${formatDate(dateRange.start, "MMM d")} - ${formatDate(dateRange.end, "MMM d, yyyy")}`
                : filteredSnapshots.length > 0
                ? `${formatDate(filteredSnapshots[filteredSnapshots.length - 1].periodStart, "MMM d")} - ${formatDate(filteredSnapshots[0].periodEnd, "MMM d, yyyy")}`
                : "No date range"}
            </span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-white border border-[#ECECF4] rounded-[20px] p-5">
            <h3 className="text-base font-bold text-[#111827]">Platform Performance</h3>
            <p className="text-[13px] text-[#6B7280] mt-0.5 mb-4">Performance by platform</p>
              ...
              <div className="space-y-3">
                {platformBreakdown.map((p) => (
                  <div key={p.platform} className="flex items-center gap-3">
                    <PlatformIcon platform={p.platform as Platform} className="w-4 h-4 shrink-0" />
                    <span className="text-[13px] font-medium text-[#6B7280] w-20">{p.label}</span>
          ...
          <div className="bg-white border border-[#ECECF4] rounded-[20px] p-5">
            <div className="flex items-center justify-between mb-0.5">
              ...
              <Select value={sortMetric} onValueChange={(val) => val && setSortMetric(val as SortMetric)}>
            ...
            <p className="text-[13px] text-[#6B7280] mb-4">Sorted by {CHART_METRIC_LABELS[sortMetric as ChartMetric] || sortMetric}</p>
            {sortedTopContent.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {sortedTopContent.map((item) => {
                  const maxVal = sortedTopContent.reduce((max, c) => Math.max(max, getSortMetricValue(c, sortMetric)), 0);
                  const metricVal = getSortMetricValue(item, sortMetric);
                  const pct = maxVal > 0 ? (metricVal / maxVal) * 100 : 0;
                  const typeColor = CONTENT_TYPE_COLORS[item.contentType] || "bg-[#F5F5F5] text-[#6B6B80]";
                  return (
                    <div key={item.id} className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg ${typeColor.split(" ")[0]} flex items-center justify-center shrink-0`}>
                        <FileText className={`w-3.5 h-3.5 ${typeColor.split(" ")[1]}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-[13px] font-semibold text-[#111827] truncate">{item.title}</span>
                          <PlatformIcon platform={item.platform as Platform} className="w-3 h-3 shrink-0 text-[#9CA3AF]" />
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <div className="w-16 h-1 rounded-full bg-[#F4F4FA] mt-0.5 ml-auto">
                          <div className="h-full rounded-full bg-[#C5F135]" style={{ width: `${Math.max(pct, 2)}%` }} />
                        </div>
                      </div>
                    </div>
                  );
        ...
        {/* Future Sections — Content Type Breakdown */}
        <div className="grid grid-cols-2 gap-5">
          <div className="bg-white border border-[#ECECF4] rounded-[20px] p-5">
            <h3 className="text-base font-bold text-[#111827]">Content Type Breakdown</h3>
            <p className="text-[13px] text-[#6B7280] mt-0.5 mb-4">Performance by content format</p>
            <div className="flex items-center justify-center h-[160px] text-sm text-[#9CA3AF] border border-dashed border-[#ECECF4] rounded-xl">
              Coming soon
            </div>
          </div>
          <div className="bg-white border border-[#ECECF4] rounded-[20px] p-5">
            <h3 className="text-base font-bold text-[#111827]">Product Performance</h3>
            <p className="text-[13px] text-[#6B7280] mt-0.5 mb-4">Most promoted products</p>
            <div className="flex items-center justify-center h-[160px] text-sm text-[#9CA3AF] border border-dashed border-[#ECECF4] rounded-xl">
              Coming soon
            </div>
          </div>
        ...
        {filterClient === "all" && (
          <div className="bg-white border border-[#ECECF4] rounded-[20px] p-5">
            ...
            <p className="text-[13px] text-[#6B7280] mt-0.5 mb-4">Performance across all clients</p>
